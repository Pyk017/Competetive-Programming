# Problem Statement:-  Chacha is playing an arcade game and wants to climb to the top of the leader board and wants to track her ranking. 
# 					The game uses Dense Ranking so its leader board works like this:

# 					The player with the highest score is ranked number 1 on the leader board.
# 					Players who have equal scores receive the same ranking number, and the next player(s) receive the immediately following ranking number.

# For example, the four players on the leader board have high scores of 100, 90, 90, and 80. Those players will have ranks 1, 2, 2, and 3, respectively. 
# If Chacha’s scores are 70, 80 and 105, her rankings after each game are 4th, 3rd and 1st.
...
chacha_score = list(map(lambda x: -int(x), input().split()))
...
for i in chacha_score:
	t = bisect.bisect_left(d, i)
	print(t + 1)
