...
Similarly, there are Y rivers flowing in a vertical direction, each flowing in a different column and covering the
entire column.
The people want to build houses of dimensions S×S on the remaining land.
What is the maximum number of houses of the given dimensions that could be built on the remaining land, such that no
part of any house is in a river and no two houses overlap?

Input:
The first line contains T, the number of test cases. Then the test cases follow.
For each test case, the first line contains N and M.
Then the second line contains X , Y and S denoting the number of rivers flowing horizontally, number of rivers flowing
vertically and the dimension of the house.
...
def promised_land(n, m, x, y, s, x_ele, y_ele):
    x_ele.append(n + 1)
    y_ele.append(m + 1)
    result_1 = result_2 = 0
    temp = 1
...
for _ in range(int(input())):
    ...
    if X > 0:
        x_elements = list(map(int, input().split()))
    if Y > 0:
        y_elements = list(map(int, input().split()))
    print(promised_land(N, M, X, Y, S, x_elements, y_elements))
